 ...
 * original `vibewave://watch?v=<id>` request, and a cache hit never resolves a
 * URL at all.
 */
@UnstableApi
object AudioCache {
     ...
     * [StreamResolver][com.music.vibewave.data.innertube.StreamResolver]'s
     ...
     * `vibewave://watch?v=…` URI it builds for itself carries an id and nothing
     * else. It rides along from the queue instead — see
     * [PlaybackService.prefetchAround][com.music.vibewave.playback.PlaybackService].
     */
     ...
     * [PlaybackService][com.music.vibewave.playback.PlaybackService]'s audition.
    ...
    private suspend fun fetch(videoId: String, position: Long, length: Long) =
        fetch(videoId, Uri.parse("vibewave://watch?v=$videoId"), position, length)
