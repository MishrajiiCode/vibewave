...
object LocalMediaRepository {
    ...
    /**
     * Retrieves all songs in the `Music/VibeWave` directory, combining app downloads
    ...
    suspend fun getDownloadedSongs(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        }.onFailure { Log.w(TAG, "Failed scanning Music/VibeWave directory: ${it.message}") }
    ...
    suspend fun getLocalMusic(context: Context): List<Song> = withContext(Dispatchers.IO) {
        ...
        // whatever this app has downloaded into Music/VibeWave alongside
