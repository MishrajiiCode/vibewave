...
import com.music.vibewave.data.sources.addon.AddonNotFound
import com.music.vibewave.data.sources.addon.AddonStream
import com.music.vibewave.data.sources.addon.AddonTrack
import com.music.vibewave.playback.StreamContainer
...
class AddonSource(
     ...
     * [QualityUpgrade][com.music.vibewave.playback.QualityUpgrade] checks a
     ...
     * [TrackMatcher][com.music.vibewave.data.sources.TrackMatcher] decides which
    ...
    private fun openable(url: String, answer: AddonStream, trackId: String, tier: String): SourceStream? {
        ...
        if (answer.isEncrypted) {
            TrackLog.w(
                TAG,
                "${config.displayName}: $trackId came back encrypted, which VibeWave never asked for — passing",
            )
            return null
        }
