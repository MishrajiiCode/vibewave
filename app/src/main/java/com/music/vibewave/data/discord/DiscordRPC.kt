package com.music.vibewave.data.discord
 ...
 *
 * ```
 *   Listening to VibeWave          <- activityName, or the app's own name
 *   ┌────┐  Song title             <- details
 *   │art │  Artist                 <- state
 *   └────┘  ▁▁▁▁▁▁ 1:04 / 3:47     <- from the timestamps
 *   [ Listen on YouTube Music ]    <- button 1
 *   [ Visit VibeWave           ]   <- button 2
 * ```
 */
class DiscordRPC(
    val context: Context,
     ...
     * a side-by-side dev install should still look like vibewave to everyone
     * else on Discord.
     */
    ...
    companion object {
        ...
        const val PROJECT_URL = "https://github.com/MishrajiiCode/VibeWave"

        const val DEFAULT_BUTTON_1 = "Listen on YouTube Music"
        const val DEFAULT_BUTTON_2 = "Visit VibeWave"
        ...
        private const val FALLBACK_ART_URL =
            "https://raw.githubusercontent.com/MishrajiiCode/VibeWave/main/app/src/main/ic_launcher-playstore.png"
