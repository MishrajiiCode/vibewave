...
void AppendEnergyCurve(std::string& out, const std::vector<vibewave::smart::EnergyPoint>& points) {
  out += '[';
...
void AppendCuePoints(std::string& out, const std::vector<vibewave::smart::MixCuePoint>& points) {
  out += '[';
  for (size_t index = 0; index < points.size(); ++index) {
    if (index > 0) out += ',';
    out += "{\"t\":";
...
Java_com_music_vibewave_playback_smart_TrackFeatures_nativeAnalyze(
    JNIEnv* env,
    jclass /* clazz */,
    jfloatArray samples,
    jdouble sample_rate,
  ...
  const vibewave::smart::AnalysisResult result =
      vibewave::smart::AnalyzeAudio(input, sample_rate, duration);
...
Java_com_music_vibewave_playback_smart_TrackFeatures_nativeSampleRate(
    JNIEnv* /* env */,
    jclass /* clazz */) {
...
Java_com_music_vibewave_playback_smart_TrackFeatures_nativeResample(
    JNIEnv* env,
    jclass /* clazz */,
    jfloatArray samples,
    jdouble input_rate,
